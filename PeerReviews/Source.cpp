...

{
	...
	std::for_each(player.inventory.begin(), player.inventory.end(),

		[](const std::pair<Item::Type, Item>& item)
		{
		...
		std::for_each(monsters.begin(), monsters.end(), [&](Object& monster)
			{
				...
				if (&monster == &monsters[monsterNum - 1])

				{

					defend(monster, attack(player));
